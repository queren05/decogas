...
- **Repositorio:** `github.com/queren05/decogas`. El `gh` CLI está autenticado en
  el PC del equipo.
- **URL de producción:** `https://queren05.github.io/decogas/` (el sitio se sirve bajo la
  subruta `/decogas/`).
- **Lo que se publica:** el build de Astro, `web/dist` (generado por el CI a partir de
  `web/`; nunca se versiona).

## Publicar: push a `main` → GitHub Actions → Pages

El despliegue es **automático y continuo**, sin pasos manuales:

1. Haz `push` (o merge) a la rama **`main`**.
2. El workflow `.github/workflows/pages.yml` se dispara: instala dependencias en `web/`
   (`npm ci`, Node 22), construye el sitio (`npm run build` → `web/dist`) y lo publica en
   GitHub Pages con las acciones oficiales
...
> **Netlify es historia.** El sitio estuvo alojado en Netlify; el `netlify.toml` y el
...
## Cabeceras de seguridad: NO hay cabeceras personalizadas en Pages

El hosting anterior (Netlify) aplicaba un archivo `_headers` con cabeceras de seguridad.
**GitHub Pages no soporta cabeceras HTTP personalizadas**, así que ese mecanismo se perdió
con el cambio de hosting (el archivo se eliminó del repo junto con la carpeta antigua).
...
En `web/public/config.js`, pon la URL del proyecto y la anon key
...
- **`supabase/setup-supabase-v5.sql`** — añade la columna `img` a
  ...
  `admin.js:402`.
- **`supabase/setup-supabase-v6-seguridad.sql`** — endurece las
...
- **`supabase/arreglo-storage-admin.sql`** — corrige el caso anterior: recrea las tres
...
> existe** (creada por v3) y no crea la tabla `leads` (la crea v4). Si vas a montar el
...
- **Bucket `productos` (Storage)** — lectura pública; la **escritura** está restringida al
  **email del administrador**, no a cualquier usuario autenticado. `setup-supabase-v5.sql` deja
  ...
  `arreglo-storage-admin.sql` la endurecen a un único email (comprobando
