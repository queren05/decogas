...
>
> Rutas con espacios y paréntesis citadas siempre entre comillas.
...
└── "decogas-web (2)"                     · carpeta entregada (nombre de descarga)
    ...
    └── "decogas-web"                     · === CARPETA DESPLEGABLE (la publica GitHub Pages) ===
        ...
        ├── termos.html                   · catálogo de termos/calentadores
        ├── calcula.html                  · calculadora de potencia + presupuesto al instante
        ├── guias.html                    · blog/guías (contenido estático propio)
        ├── legal.html                    · aviso legal / privacidad
        ├── admin.html                    · panel de administración del catálogo (login)
        ├── clientes.html                 · panel de leads/solicitudes (login)
        │
        ...
        ├── admin.js                      · CRUD del catálogo contra Supabase (Auth + tabla products + Storage)
        ├── clientes.js                   · lectura/borrado de leads contra Supabase
        │
        ├── data-calderas.js              · dataset fallback de calderas (13 modelos) + registro en DECOGAS_DATASETS
        ├── data-aires.js                 · dataset fallback de aires (19 modelos)
        ├── data-termos.js                · dataset fallback de termos (vacío: solo vienen de Supabase)
        ├── data-guias.js                 · contenido de las guías (fuente única, no hay tabla en BD)
        │
...
- **Escapado XSS** (`esc()`) en cada dato antes de inyectarlo en el DOM — está
  duplicado en casi todos los ficheros.
